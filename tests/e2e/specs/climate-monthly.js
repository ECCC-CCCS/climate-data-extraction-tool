...
describe("E2E test for climate monthly data with various form options", () => {
  it("Check climate monthly stations and download data as CSV", () => {
    ...
    cy.intercept(
      "GET",
      /.*\/collections\/climate-stations\/items\?.*HAS_MONTHLY_SUMMARY=Y.*f=json.*offset=0.*/
    ).as("stationData");
    cy.intercept(
      "GET",
      /.*\/collections\/climate-monthly\/items\?.*sortby=LOCAL_DATE&limit=1.*f=json.*/
    ).as("dateRangeData");
    ...
    cy.waitUntil(
      () =>
        cy.wait("@stationData").then((xhr) => {
          expect(xhr.response.headers).to.have.property(
          ...
          try {
            ...
            expect(xhr.response.headers["content-encoding"]).to.match(/gzip/gi);
          } catch {
            cy.log(
              "content-encoding does not exist in response header. Test continued."
            );
          }
    ...
    cy.intercept("GET", /.*\/collections\/climate-monthly\/items.*/).as(
      "countData"
    );
    ...
    cy.get("#oapif-link-list").find("a").should("have.length.of.at.most", 200); // actual 191
    cy.get("#oapif-link-list a:first")
      ...
      .then((href) => {
        ...
        cy.request("GET", hrefLimited).then((response) => {
          try {
            ...
            expect(response.headers["content-encoding"]).to.match(/gzip/gi);
          } catch {
            cy.log(
              "content-encoding does not exist in response header. Test continued."
            );
          ...
          expect(response.body)
            .to.include("x,y")
            .to.include("STATION_NAME")
            .to.include("CLIMATE_IDENTIFIER")
            .to.include("ID");
        });
      });
  });
  ...
  it("Download data as GeoJSON by province", () => {
    ...
    // date change
    cy.inputText("input#date-start-date", "1899-01{enter}").wait(2000);
    cy.inputText("input#date-end-date", "2020-12{enter}").wait(2000);
    ...
    cy.intercept(
      "GET",
      /.*\/collections\/climate-monthly\/items\?.*PROVINCE_CODE=BC.*resulttype=hits.*f=json.*/
    ).as("countData");
    cy.get("#retrieve-download-links").scrollIntoView().wait(250).click();
    cy.waitUntil(
      () =>
  ...
  it("Download data as GeoJSON by a select few stations", () => {
    ...
    // date change
    cy.inputText("input#date-start-date", "2000-01{enter}").wait(2000);
    cy.inputText("input#date-end-date", "2010-12{enter}").wait(2000);

    // Select stations by table
    cy.get("table#station-select-table").scrollIntoView().wait(250);
    cy.get(
      "table#station-select-table tr.selectable:contains(1101158):first"
    ).click();
    cy.get(
      "table#station-select-table tr.selectable:contains(6158667):first"
    ).click();
    cy.get(
      "table#station-select-table tr.selectable:contains(7027322):first"
    ).click();
  ...
  it("Download data as GeoJSON by a zoomed BBOX", () => {
    ...
    cy.get("#clear-selected-stations")
      .scrollIntoView()
      .wait(250)
      .click({ force: true })
      .wait(500);
    ...
    // date change
    cy.inputText("input#date-start-date", "2010-01{enter}").wait(2000);
    cy.inputText("input#date-end-date", "2020-12{enter}").wait(2000);
