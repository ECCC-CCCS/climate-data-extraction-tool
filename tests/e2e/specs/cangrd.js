...
describe("E2E test for CANGRD ogc-api-coverage data with various form options", () => {
      ...
      .then((href) => {
        ...
        cy.request("GET", href).then((response) => {
          ...
          expect(response.headers["content-disposition"]).to.match(
            /.*CANGRD_hist_annual_anom_ps50km_TMEAN_1960-1962\.tif.*/
          );
        });
      });
  });
  ...
  it("Download data as tmin, anomaly, monthly, 1999-11 to 1999-12, GeoTIFF", () => {
    ...
    // Date range
    cy.inputText("#date-start-date", "1999-11{enter}").wait(1000);
    cy.get("input#date-end-date").should("not.be.disabled");
    ...
    cy.get("a#download-url")
      ...
      .then((href) => {
        ...
        cy.request("GET", href).then((response) => {
          expect(response.status).to.equal(200);
          expect(response.headers["content-disposition"]).to.match(
            /.*CANGRD_hist_monthly_anom_ps50km_TMIN_1999-11-1999-12\.tif.*$/
          );
        });
      });
  });
