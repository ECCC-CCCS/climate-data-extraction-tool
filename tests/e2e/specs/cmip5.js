...
describe('E2E test for CMIP5 page with ogc-api-coverage data', () => {
  ...
  it('Download data as tas, RCP2.6, Annual, Anomaly, 50, 2010-2050, CoverageJSON', () => {
    cy.visit('/#/cmip5-data')

    // variable
    cy.selectVar('#var-sel-variable', 'Mean temperature', 'tas')
    ...
    // visit download link
    cy.get('#oapi-download-links-list').scrollIntoView().wait(250).should('be.visible')
    cy.get('#oapi-download-links-list a:first').should('have.attr', 'href').then((href) => {
  ...
  it('Download data as sfcWind, historical, monthly, 1990-01 to 1999-12, CoverageJSON', () => {
    cy.visit('/#/cmip5-data')
  ...
  it('Download data as Snow depth, RCP8.5, Monthly, 2010-01 to 2019-12, NetCDF', () => {
    cy.visit('/#/cmip5-data')
    ...
    cy.get('#oapi-download-links-list a:first').should('have.attr', 'href').then((href) => {
      cy.request('GET', href).then((response) => {
        expect(response.status).to.equal(200)
        expect(response.headers['content-type']).to.equal('application/x-netcdf')
        expect(response.headers['content-disposition']).to.match(/.*CMIP5_rcp8\.5_monthly_abs_latlon1x1_snd_pctl50_P1M\.nc.*$/)
      })
    })
  })
})
