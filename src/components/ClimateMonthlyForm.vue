...
export default {
  ...
  data () {
    return {
      ...
      date_start: this.$moment.utc('1969-01-01 00:00:00', 'YYYY-MM-DD HH:mm:ss').toDate(),
      date_end: this.$moment.utc().toDate(),
      date_min: this.$moment.utc('1969-01-01 00:00:00', 'YYYY-MM-DD HH:mm:ss').toDate(),
  ...
  beforeMount () {
    ...
    if (this.climateStationsGeoJson === null) { // prevent duplicate AJAX
      var url = this.wfs3_url_base + '/' + this.wfs_layer_station + '/items?HAS_MONTHLY_SUMMARY=Y&limit=' + this.wfs_station_limit
      this.$store.dispatch('retrieveClimateMonthlyStations', url)
    }
